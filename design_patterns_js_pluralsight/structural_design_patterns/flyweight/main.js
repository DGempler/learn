...
// Typically would use getters & setters to not expose fact that you're using flyweights
...
var FlyweightFactory = function(){
    var flyweights = {};
    // this.name is the only unique item on tasks
    var get = function(project, priority, user, completed) {
        if (!flyweights[projects + priority + user + completed]) {
            flyweights[projects + priority + user + completed] =
                new Flyweight(project, priority, user, completed);
        }
        return flyweights[projects + priority + user + completed];
    };
    var getCount = function() {
        var count = 0;
